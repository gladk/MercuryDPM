...
class HourGlass2D : public HGRID_3D{
public:
    ...
    void setup_particles_initial_conditions()
    {
        ...
        Points[0] = Vec3D(get_xmin()                 ,0.0,(get_zmin()+get_zmax())/2.0+ContractionHeight);
        Points[1] = Vec3D(get_xmin()+ContractionWidth,0.0,(get_zmin()+get_zmax())/2.0                  );
        Points[2] = Vec3D(get_xmin()                 ,0.0,(get_zmin()+get_zmax())/2.0-ContractionHeight);
        w1.create_open_prism_wall(Points);
        getWallHandler().copyAndAddWall(w1);
		//right neck wall
        Points[0] = Vec3D(get_xmax()                 ,0.0,(get_zmin()+get_zmax())/2.0+ContractionHeight);
        Points[1] = Vec3D(get_xmax()-ContractionWidth,0.0,(get_zmin()+get_zmax())/2.0                  );
        Points[2] = Vec3D(get_xmax()                 ,0.0,(get_zmin()+get_zmax())/2.0-ContractionHeight);
        w1.create_open_prism_wall(Points);
        getWallHandler().copyAndAddWall(w1);

		// The above wall definition is equivalent to
		//~ FiniteWall w1;
        //~ Vec3D A(get_xmin()                 ,0.0,(get_zmin()+get_zmax())/2.0+ContractionHeight);
        //~ Vec3D B(get_xmin()+ContractionWidth,0.0,(get_zmin()+get_zmax())/2.0                  );
        //~ Vec3D C(get_xmin()                 ,0.0,(get_zmin()+get_zmax())/2.0-ContractionHeight);
        ...
        for (double x=MaxParticleRadius; x<get_xmax(); x+=1.999*MaxParticleRadius) 
        {
			p0.set_Radius(random.get_RN(MinParticleRadius,MaxParticleRadius));
            p0.set_Position(Vec3D(x, 0.0, z+p0.get_Radius()));
